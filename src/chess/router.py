...
from .services import PlayGameService, UserProfileService, UserStatsService
...
@router.get(
    "/profiles/{profile_id}/full", summary="Get a full user profile with stats by id", response_model=UserProfileFullOut
)
...
@router.get(
    "/profiles/{profile_id}/stats", summary="Get all user stats by profile id", response_model=list[UserStatsOut]
)
async def get_user_stats(
    ...
    if stats is None:
        raise HTTPException(status_code=404, detail="User stats not found") from None
    return stats


@router.get(
    "/profiles/{profile_id}/stats/{game_type}",
    summary="Get user stats by profile id and game type",
...
async def record_result(
    ...
    if result is None:
        raise HTTPException(status_code=404, detail="One or both user profiles not found") from None
    return result
